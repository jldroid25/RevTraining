...
public class ArraysTask {
	/*
	 * Write a program in java using arrays to compute the average of n grades. Ask
	 * the user how many grades to enter i e 10 grades, then lst the user save the
	 * grades, then program to calculate the average grade entered by user . Use a
	 * separate method on the class for the program, but use scanner on the main
	 * method to get input from user, & call the program.
	 */
	public static int averageGrades(int[] grades, int arraySizeNumber) {
		int sumResults = 0;
		for (int i = 0; i < arraySizeNumber; i++) {
	...
	/**
	 * An irregular array is a "n-dimensianal array" ( where n is greater than 1) This
	 * kind of 1-D array the last dimension's size is unknown. so Memory for this
	 * one-D array is alloacted with/of different size When declaring
	 * irregular-Array at least one dimension will not contain a specfic value.
	 */
	// Irregular Arrays Program
	public static void myIrregularArray() {
		// Total number of elements are unknown at this point.
	...
	/**
	 * VarArgs allows us to set an argument to a method whose size is determined at
	 * runtime. You can only ever have 1 varargs parameter in a method, and it MUST
	...
	public static void main(String[] args) {
		...
		// Store User desired size into a grade array
		int[] gradesArray = new int[sizeOfArray];
>>>>>>> ac74dcc6311f459cd2576ae846f153a1fd804133
		...
		myIrregularArray();
		// Calling irregular Variable Arguments MEthod
		variableArgumentsMethod("VTSAX", "VOO", "VTI", "VNQ", "VXUS", "VCoin", "VJones", "VTRRKX", "VS&P 500",
				"VTherium");
		int matrix1[][] = { { 15, 5, 25 }, { 2, 1, 2 }, { 8, 9, 6 } };
		int matrix2[][] = { { 35, 1, 6 }, { 3, 11, 3 }, { 12, 21, 121 } };
		// Calling the Adding Matrices Method(
